...
export default function InsightsPage() {
  const posts: ContentSummary[] = getAllContent('insights')
      ...
      <section className="bg-white py-16">
        <Container>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <a
              href="/api/rss"
              className="flex items-center gap-1.5 text-xs text-slate-500 transition-colors hover:text-blue-700"
              aria-label="RSS feed"
            >
            ...
            <div className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {posts.map((post) => (
                <Link
                  ...
                  className="group flex flex-col rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-blue-200 hover:shadow-md"
                >
                  ...
                  <h2 className="mt-4 text-lg font-semibold leading-snug text-slate-900 transition-colors group-hover:text-blue-700">
                    {post.title}
                  </h2>
                  <p className="mt-2 line-clamp-3 flex-1 text-sm leading-relaxed text-slate-600">
                    {post.description}
                  </p>
                  <div className="mt-5 flex items-center gap-3 border-t border-slate-100 pt-4 text-xs text-slate-400">
                    {post.author && (
